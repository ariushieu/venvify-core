...
### 2.4 Enum — thêm giá trị (KHÔNG cần migration DDL)
Vì đều lưu `@Enumerated(STRING)` vào cột `VARCHAR(20)` có sẵn → chỉ sửa code Java, schema không đổi.
- `TransactionType` += `TICKET_RESALE`
- `PaymentProvider` += `SEPAY`
- `BookingStatus`: đã có `RESERVED` rồi → không thêm.
...
## 5. Migration V2 (phác thảo — chốt DDL lúc implement)
File `src/main/resources/db/migration/V2__ticket_transfer_and_payment.sql`:
```sql
-- bookings
