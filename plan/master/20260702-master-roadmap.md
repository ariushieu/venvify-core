...
|---|---|---|---|---|
| **O-MP1** | ~~Media: LiveKit self-host vs mediasoup vs cloud — kéo theo còn cần Node riêng không~~ **CHỐT 2026-07-09: GIỮ Node service riêng** (user quyết, ngược khuyến nghị cũ) | ~~LiveKit self-host + Spring WS, bỏ Node~~ → **giữ `venvify-realtime` (Node + Socket.IO) làm relay thuần** | ✅ chốt 2026-07-09 | User giữ kiến trúc microservice (SPEC v1.0 gốc): Node relay signaling + chat/poll/Q&A, verify JWT của core (HS shared key), hỏi core internal API để authorize room. **Media SFU (LiveKit vs mediasoup vs cloud) VẪN mở** — chốt bằng PoC đầu P4; Node relay không tự làm SFU. Skeleton service (`../../venvify-realtime`) dựng 2026-07-09: server bootstrap + JWT handshake auth + room gateway seam + Spring client port, chưa có nghiệp vụ room (chờ core mở internal room-authorize endpoint ở P4). |
